...
import '../ApiConfiguration/api/api_service.dart';
...
class TaskApiService {
  ...
  Future<void> initialize() async {
    _cachedToken = await _apiService.getToken();
  }
  ...
  Future<Map<String, dynamic>?> getDashboard() async {
    try {
      final response = await _apiService.get(endpoint: '/api/dashboard');
  ...
  Future<TaskItem?> getTaskDetails(String taskId) async {
    try {
      final response = await _apiService.get(endpoint: '/api/tasks/$taskId');
      if (response.statusCode == 200) {
  ...
  Future<List<TaskNote>?> getNotes(String taskId) async {
    try {
      final response = await _apiService.get(endpoint: '/api/tasks/$taskId/notes');
      if (response.statusCode == 200) {
        final rawData = response.data;
        List list = [];
        if (rawData is Map) {
  ...
  Future<bool> addNote(String taskId, String content) async {
    try {
      final response = await _apiService.post(
        endpoint: '/api/tasks/$taskId/notes',
        data: {
          'note': content.trim(),
  ...
  Future<String?> uploadTaskImage(String taskId, String filePath) async {
    try {
      ...
      final response = await _apiService.postMultipart(
        endpoint: '/api/tasks/$taskId/images/upload',
        formData: formData,
      );
